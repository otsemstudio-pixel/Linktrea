...
export default function PositionsHistory({ positions }: Props) {
  ...
  return (
    <section className="px-6 py-6 border-t border-ink-raised @min-[1024px]:px-0">
      <h2 className="text-label uppercase tracking-label text-muted mb-4">Historique des positions</h2>
              ...
              <li key={position.id} className="relative pl-7 pb-6 last:pb-0">
                ...
                >
                  <div className="flex justify-between items-baseline gap-2">
                    <span className="font-mono text-xs text-muted tabular-nums">{displayYearRange(position)}</span>
                  ...
                  {position.highlights.length > 0 && (
                    <ul className="mt-2 flex flex-col gap-1">
                      {position.highlights.slice(0, 3).map((highlight, hi) => (
                        <li key={hi} className="text-sm text-muted flex gap-1.5">
